<div class="row">
    ...
    <div class="col-md-4 form-group {{$errors->has('value') ? 'has-error' : ''}} ">
        ...
        <input type="text" class="form-control" value="{{old('recurrence_days', $plan->exists() ? $plan->value : '')}}"
               name="value" id="value" placeholder="Valor do plano">
        @if($errors->has('value'))
    ...
    <div class="col-md-4 form-group {{$errors->has('plan_duration') ? 'has-error' : ''}} ">
        ...
        <select class="form-control " id="plan_duration" name="plan_duration">
            ...
            <option value="30" {{$plan->exists() && $plan->plan_duration == 'infinity' ? 'selected' : ''}}>Infinita
            </option>
    ...
    <div class="col-md-3 form-group {{$errors->has('total_retry') ? 'has-error' : ''}}">
        ...
        <select type="text" name="total_retry" class="form-control {{$errors->has('total_retry') ? 'is-invalid' : ''}}"
                id="total_retry">
            <option value="">Selecione</option>
    ...
    <div class="col-md-3 form-group {{$errors->has('retry_after_day') ? 'has-error' : ''}}">
        ...
        <select type="text" name="retry_after_day"
                class="form-control {{$errors->has('retry_after_day') ? 'is-invalid' : ''}}" id="total_retry">
            <option value="">Selecione</option>
    ...
    <div class="col-md-3 form-group {{$errors->has('type') ? 'has-error' : ''}}">
        ...
        <select type="text" name="type" class="form-control {{$errors->has('type') ? 'is-invalid' : ''}}" id="type">
            ...
            <option value="credit_card" {{$plan->exists() && $plan->type == 'credit_card' ? 'selected' : ''}}>Cartão
            </option>
    ...
    @foreach($fieldsUpdate as $field)
        <div class="col-md-4 form-group ">
            <div class="input-group">
                ...
                <input value="{{old($field['name'], $plan->exists()  ? $plan->{$field['name']} : '')}}" type="text"
    ...
    <div class="col-md-12 form-group " style="">
        <div class="input-group">
            ...
            <textarea name="description" class="form-control" id="description"
                      maxlength="500">{{$plan->exists() && $plan->description  ? $plan->description : ''}}</textarea>
        </div>
    </div>
