    ...
    <div class="col-md-12 form-group {{$errors->has('resources') ? 'has-error' : ''}} ">
        ...
        <select class="form-control " id="selectResource" name="resources[]" multiple="multiple">
            ...
            @foreach($resources as $resource)
                ...
                <option class="profilesSelect" data-route="{{$nameRoute[0]}}"
                        {{$profile->exists && in_array($resource->id,$profilesResources) ? 'selected="selected"' : '' }} value="{{$resource->id}}">{{$resource->name}}</option>
            @endforeach
        </select>
